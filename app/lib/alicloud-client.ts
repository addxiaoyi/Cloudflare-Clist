...
export class AliyunDriveClient {
  ...
  private async refreshToken(): Promise<void> {
    ...
    throw lastError instanceof Error
      ? lastError
      : new Error('Aliyun refresh failed');
  }
  ...
  private async refreshTokenOnline(): Promise<void> {
    ...
    const url = new URL(
      getConfigString(
        ...
        DEFAULT_ALICLOUD_API_ADDRESS,
      ),
    );
    url.searchParams.set('refresh_ui', refreshToken);
    url.searchParams.set('server_use', 'true');
    const driverTxt =
      this.config.alipan_type === 'alipanTV' ? 'alicloud_tv' : 'alicloud_qr';
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Aliyun online refresh failed: ${response.status} ${text}`,
      );
    }
    ...
    if (!data.access_token || !data.refresh_token) {
      throw new Error(
        data.text || 'Aliyun online refresh returned empty token',
      );
    }
  ...
  private async loadDriveInfo(): Promise<void> {
    await this.ensureAccessToken();
    const data = await this.requestJsonRaw(
      API_ENDPOINTS.GET_DRIVE_INFO,
      'POST',
      {},
    );
  ...
  async getObject(
    ...
    const response = (await this.requestJson(
      API_ENDPOINTS.FILE_GET_DOWNLOAD_URL,
      'POST',
      {
        drive_id: this.saving.drive_id,
        file_id: fileId,
    ...
    const url =
      response.url || streams?.[this.config.livp_download_format || 'jpeg'];
    if (!url) {
  ...
  async deleteObject(key: string): Promise<void> {
    ...
    const endpoint =
      this.config.remove_way === 'delete'
        ? API_ENDPOINTS.FILE_DELETE
        : API_ENDPOINTS.FILE_TRASH;
  ...
  async initiateMultipartUpload(
    ...
    const partInfoList = Array.from({ length: partCount }, (_, idx) => ({
      part_number: idx + 1,
    }));
  ...
  async uploadPart(
    ...
    const partInfo =
      (state.partInfo as Array<{ part_number: number; upload_url: string }>) ||
      [];
