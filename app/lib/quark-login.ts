...
export async function casRequest(
...
export async function exchangeTicketForCookie(
  ticket: string,
  jar: CookieJar,
  signal?: AbortSignal,
): Promise<string> {
  ...
  const resp = await fetchTrack(
    infoUrl,
    { Accept: 'application/json, text/plain, */*', Referer: `${PAN_HOST}/` },
    jar,
    signal,
  );
  if (resp.status >= 400) {
...
export async function queryQrSession(
  session: QrSession,
  signal?: AbortSignal,
): Promise<QueryResult> {
  ...
  if (members?.service_ticket) {
    const cookie = await exchangeTicketForCookie(
      members.service_ticket,
      jar,
      signal,
    );
    if (cookie) {
