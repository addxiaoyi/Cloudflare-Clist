...
export function describeError(
  ...
  if (status === 401)
    return `${platformLabel} 认证失败（401）：Token 无效或已过期`;
  if (status === 403) {
    const hint = /rate limit/i.test(body)
      ? '触发限流，请稍后重试'
      : 'Token 缺少所需权限';
    return `${platformLabel} 访问被拒（403）：${hint}`;
  }
  if (status === 404)
    return `${platformLabel} 未找到目标（404）：仓库、分支或路径不存在`;
  if (status === 422)
    return `${platformLabel} 校验失败（422）：${body || '请求体不合法'}`;
