...
async function json<T>(
  ...
  if (!res.ok)
    throw new Error(describeError('GitLab', res.status, await res.text()));
...
async function rawBytes(
  ...
  const url = withQuery(
    conn,
    `/projects/${projectId(conn)}/repository/files/${encodeFilePath(repoPath)}/raw`,
    { ref: conn.branch },
  );
...
export const gitlabAdapter: GitPlatformAdapter = {
  ...
  buildConnection(config) {
    ...
    if (!segs || segs.length < 2)
      throw new Error('GitLab 存储需填写项目路径（如 group/subgroup/project）');
    const token = typeof cfg.token === 'string' && cfg.token ? cfg.token : '';
    if (!token)
      throw new Error(
        'GitLab 存储需填写 Personal Access Token 或 Project Access Token',
      );
    let rawApiBase =
  ...
  async headCommit(conn) {
    const commits = await json<CommitLite[]>(
      conn,
      withQuery(conn, `/projects/${projectId(conn)}/repository/commits`, {
        ref_name: conn.branch,
        per_page: '1',
      }),
    );
    ...
    return {
      sha: lite?.id || '',
      date: lite?.created_at || new Date().toISOString(),
    };
  },
  ...
  async listDir(conn, repoPath, opts) {
    ...
    while (true) {
      const res = await request(
        conn,
        withQuery(conn, `/projects/${projectId(conn)}/repository/tree`, {
          ...params,
          page: String(page),
        }),
      );
  ...
  async statFile(conn, repoPath) {
    ...
    const res = await request(
      ...
      withQuery(
        conn,
        `/projects/${projectId(conn)}/repository/files/${encodeFilePath(repoPath)}`,
        { ref: conn.branch },
      ),
    );
  ...
  async deleteFile(conn, repoPath, message) {
    ...
    await json(
      ...
      withQuery(
        conn,
        `/projects/${projectId(conn)}/repository/files/${encodeFilePath(repoPath)}`,
        { branch: conn.branch, commit_message: message },
      ),
      {
        method: 'DELETE',
      },
...
function toCommitMeta(
  arr: { id: string; created_at: string }[],
): GitCommitMeta {
  return {
    sha: arr[0]?.id || '',
    date: arr[0]?.created_at || new Date().toISOString(),
  };
}
