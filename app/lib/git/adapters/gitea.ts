...
async function json<T>(
  ...
  if (!res.ok)
    throw new Error(describeError('Gitea', res.status, await res.text()));
...
export const giteaAdapter: GitPlatformAdapter = {
  ...
  buildConnection(config) {
    const cfg = config || {};
    const segs = parseRepoSegments(
      cfg.repo,
      /(?:codeberg\.org|gitea\.com)[:/]/i,
    );
    const two = requireTwoSegments(segs);
    if (!two)
      throw new Error('Gitea 存储需填写仓库，支持 owner/repo 或完整仓库 URL');
  ...
  async headCommit(conn) {
    const ref = await json<RefResp>(
      ...
      withQuery(
        conn,
        `/repos/${repoId(conn)}/git/refs/heads/${encodeURIComponent(conn.branch)}`,
        {},
      ),
    );
  ...
  async listTree(conn) {
    ...
    const res = await request(
      conn,
      api(conn, `/repos/${repoId(conn)}/git/trees/${meta.sha}?recursive=true`),
    );
    ...
    if (tree.truncated)
      throw new Error('Gitea 仓库条目过多，Git Trees 递归列表被截断');
    const out: GitDirEntry[] = [];
  ...
  async statFile(conn, repoPath) {
    ...
    if (!res.ok)
      throw new Error(describeError('Gitea', res.status, await res.text()));
    const entry = (await res.json()) as Partial<ContentsItem> & {
      type?: string;
    };
  ...
  async readFile(conn, repoPath) {
    ...
    const res = await request(
      conn,
      api(conn, `/repos/${repoId(conn)}/git/blobs/${s.sha}`),
    );
    if (!res.ok)
      throw new Error(describeError('Gitea', res.status, await res.text()));
  ...
  signedUrl(conn, repoPath) {
    return api(
      conn,
      `/repos/${repoId(conn)}/contents/${encodePathRepo(repoPath)}?ref=${encodeURIComponent(conn.branch)}`,
    );
  },
};
