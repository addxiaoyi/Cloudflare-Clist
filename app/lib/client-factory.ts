...
export function createClient(
  ...
  if (storage.type === 'r2-oauth') {
    ...
    const accessToken =
      cfg.cloudflare_access_token ||
      cfg.access_token ||
      storage.saving?.cloudflare_access_token ||
      storage.saving?.access_token ||
      '';
    ...
    return new R2OAuthClient({
      ...
      clientId:
        cfg.client_id ||
        (env as Record<string, string | undefined>)?.CF_CLIENT_ID ||
        '',
      clientSecret:
        cfg.client_secret ||
        (env as Record<string, string | undefined>)?.CF_CLIENT_SECRET ||
        '',
  ...
  if (storage.type === 's3') {
    ...
    return new S3Client({
      ...
      basePath:
        storage.basePath || cfg.root_folder_path || cfg.base_path || '/',
      usePathStyle:
        cfg.path_style === true || cfg.use_path_style === true
          ? true
          : cfg.path_style === false || cfg.use_path_style === false
            ? false
            : undefined,
...
export function createMysqlClient(
  ...
  const connectionString =
    cfg.connection_string || cfg.endpoint || storage.endpoint || '';
  if (!connectionString && !env?.HD && !env?.HYPERDRIVE) {
    throw new Error(
      'MySQL 需先绑定 Cloudflare Hyperdrive（或本地填直连连接串）',
    );
  }
