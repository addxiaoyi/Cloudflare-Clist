...
function buildWranglerJson(state: SetupState): object {
  ...
  const cfg: Record<string, unknown> = {
    ...
    vars: {
      ...
      ADMIN_USERNAME: site.adminUsername || 'admin',
      ADMIN_PASSWORD: site.adminPassword || 'changeme',
    },
  };
  if (gdrive.enabled) {
    cfg.vars = {
  ...
  if (d1.enabled) {
    // 不写 database_id：wrangler 部署时按 database_name 自动查找或创建
    cfg.d1_databases = [
      { binding: d1.binding, database_name: d1.databaseName },
    ];
  }
  if (r2.enabled) {
...
function buildCommands(state: SetupState): string {
  const { cloudflare, d1, gdrive, webdav, github } = state;
  const lines: string[] = [];
  const hasGh = github.repoOwner && github.repoName;
  const repo = hasGh ? `${github.repoOwner}/${github.repoName}` : '';

  lines.push('# 1. Cloudflare 连接 GitHub（控制台操作，一次性）');
  lines.push(
    '#    访问 https://dash.cloudflare.com → Workers 和 Pages → 创建 → Workers → 连接 Git 仓库',
  );
  if (repo) lines.push(`#    选择仓库 ${repo}，监听分支 main`);
  lines.push(
    `#    构建设置：构建命令 npm run build；部署命令 npx wrangler deploy --config build/server/wrangler.json`,
  );
  lines.push('');
  lines.push('# 2. 推送代码，自动构建部署');
  if (repo) {
    lines.push(`git add -A && git commit -m "deploy" && git push origin main`);
  } else {
    lines.push('git add -A && git commit -m "deploy" && git push');
  }
  lines.push('');
  if (d1.enabled) {
    lines.push(
      `# 3. D1 数据库 ${d1.databaseName} 与 R2 桶：首次部署自动创建，无需手动配置`,
    );
  } else {
    lines.push('# 3. R2 桶：首次部署自动创建，无需手动配置');
  }
  lines.push('');
  lines.push('# 4. 管理员账号（默认 admin / changeme）');
  lines.push(
    '#    上线后务必在 Cloudflare 控制台 → Settings → Variables and Secrets 中修改 ADMIN_PASSWORD',
  );

  if (webdav.enabled) {
    lines.push('');
    lines.push(
      '# 5. WebDAV 凭据：控制台 Settings → Variables and Secrets 中添加 WEBDAV_USERNAME / WEBDAV_PASSWORD',
    );
  }
  if (gdrive.enabled) {
    lines.push('');
    lines.push(
      '# 5. Google Drive：GOOGLE_CLIENT_ID / GOOGLE_REDIRECT_URI 已写入 wrangler.jsonc',
    );
    lines.push(
      '#    GOOGLE_CLIENT_SECRET 在控制台 Settings → Variables and Secrets 中添加',
    );
  }
  if (cloudflare.apiToken) {
    lines.push('');
    lines.push('# 本地部署（可选，需先 wrangler login）');
...
export default function Setup({ loaderData }: Route.ComponentProps) {
  ...
  useEffect(() => {
    setState((prev) => ({
      ...
      github:
        ...
        !prev.github.repoOwner &&
        !prev.github.repoName
          ? {
              ...prev.github,
              repoOwner: loaderData.prefillRepo.split('/')[0] || '',
              repoName: loaderData.prefillRepo.split('/')[1] || '',
            }
          : prev.github,
    }));
  }, [loaderData.prefillRepo]);
    ...
    const t = setTimeout(() => {
      try {
        localStorage.setItem(
          DRAFT_KEY,
          JSON.stringify(stripSensitiveDraft(state)),
        );
      } catch {
        /* 存储不可用时忽略 */
      }
  ...
  const patch = useCallback(
    <K extends keyof SetupState>(
      section: K,
      values: Partial<SetupState[K]>,
    ) => {
      setState((prev) => ({
        ...prev,
  ...
  const wranglerJson = useMemo(
    () => JSON.stringify(buildWranglerJson(state), null, 2),
    [state],
  );
  ...
  const validation = useMemo(() => {
    const errors: string[] = [];
    if (!!state.site.adminUsername.trim() !== !!state.site.adminPassword.trim())
      errors.push(
        '管理员用户名与密码需同时填写，或都留空使用默认 admin / changeme',
      );
    else if (
      state.site.adminPassword.length > 0 &&
      state.site.adminPassword.length < 6
    )
      errors.push('管理员密码至少 6 位');
    if (
      state.cloudflare.accountId.trim() &&
      !HEX32.test(state.cloudflare.accountId.trim())
    )
      errors.push('Account ID 应为 32 位十六进制');
    if (!WORKER_NAME_RE.test(state.cloudflare.workerName))
      errors.push('Worker 名称只能含字母、数字、连字符');
    if (!DATE_RE.test(state.cloudflare.compatibilityDate))
      errors.push('兼容日期格式应为 YYYY-MM-DD');
    if (state.r2.enabled && !state.r2.bucketName.trim())
      errors.push('R2 桶名必填');
    if (state.gdrive.enabled) {
      if (!state.gdrive.clientId.trim() || !state.gdrive.clientSecret.trim())
        errors.push('Google Client ID / Secret 必填');
      if (!state.gdrive.redirectUri.trim()) errors.push('Google 回调地址必填');
      else if (!isHttpsUrl(state.gdrive.redirectUri.trim()))
        errors.push('Google 回调地址必须是合法的 HTTPS URL');
    }
  ...
  const stepErrors = useMemo(() => {
    ...
    if (current.key === 'site') {
      if (
        !!state.site.adminUsername.trim() !== !!state.site.adminPassword.trim()
      )
        errors.push(
          '管理员用户名与密码需同时填写，或都留空使用默认 admin / changeme',
        );
      ...
      if (state.gdrive.enabled) {
        if (!state.gdrive.clientId.trim() || !state.gdrive.clientSecret.trim())
          errors.push('Google Client ID / Secret 必填');
        if (!state.gdrive.redirectUri.trim())
          errors.push('Google 回调地址必填');
        else if (!isHttpsUrl(state.gdrive.redirectUri.trim()))
          errors.push('Google 回调地址必须是合法的 HTTPS URL');
      }
          ...
          {current.key === 'site' && (
            <div className="space-y-4">
              <SectionTitle
                ...
                title="站点与管理员账号"
                desc="留空则使用默认 admin / changeme，上线后请在 Cloudflare 控制台修改"
              />
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="站点标题">
                  <input
                    className={inputCls}
                    value={state.site.siteTitle}
                    onChange={(e) =>
                      patch('site', { siteTitle: e.target.value })
                    }
                    placeholder="Starx"
                  />
                </Field>
                <Field label="上传分块大小 (MB)">
                  <input
                    ...
                    min={1}
                    max={200}
                    value={state.site.chunkSizeMb}
                    onChange={(e) =>
                      patch('site', {
                        chunkSizeMb: Number(e.target.value) || 50,
                      })
                    }
                  />
                </Field>
              ...
              <Field label="站点公告（可选）">
                <input
                  className={inputCls}
                  value={state.site.announcement}
                  onChange={(e) =>
                    patch('site', { announcement: e.target.value })
                  }
                  placeholder="欢迎使用 Starx 存储服务"
                />
              </Field>
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="管理员用户名">
                  <input
                    className={inputCls}
                    value={state.site.adminUsername}
                    onChange={(e) =>
                      patch('site', { adminUsername: e.target.value })
                    }
                    placeholder="admin"
                  />
                </Field>
                <Field label="管理员密码">
                  <input
                    ...
                    value={state.site.adminPassword}
                    onChange={(e) =>
                      patch('site', { adminPassword: e.target.value })
                    }
                    placeholder="至少 6 位"
                  />
                </Field>
          ...
          {current.key === 'cloudflare' && (
            <div className="space-y-4">
              <SectionTitle
                ...
                title="Cloudflare 连接"
                desc="Git 集成部署无需 API Token；Token 仅本地部署需要"
              />
              <Field
                label="API Token（可选）"
                hint="仅本地 wrangler 部署需要：Cloudflare → My Profile → API Tokens → Edit Cloudflare Workers 模板"
              >
                <input
                  className={inputCls}
                  type="password"
                  ...
                  onChange={(e) =>
                    patch('cloudflare', { apiToken: e.target.value })
                  }
                  placeholder="可选，粘贴 API Token"
                />
              </Field>
              <Field
                label="Account ID（可选）"
                hint="仅本地部署需要，可留空自动识别"
              >
                <input
                  className={inputCls}
                  value={state.cloudflare.accountId}
                  onChange={(e) =>
                    patch('cloudflare', { accountId: e.target.value })
                  }
                  placeholder="可选，粘贴 Account ID"
                />
              </Field>
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Worker 名称">
                  <input
                    className={inputCls}
                    value={state.cloudflare.workerName}
                    onChange={(e) =>
                      patch('cloudflare', { workerName: e.target.value })
                    }
                  />
                </Field>
                <Field label="兼容日期">
                  <input
                    className={inputCls}
                    value={state.cloudflare.compatibilityDate}
                    onChange={(e) =>
                      patch('cloudflare', { compatibilityDate: e.target.value })
                    }
                  />
                </Field>
              </div>
              ...
              {state.d1.enabled && (
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="数据库名称">
                    <input
                      className={inputCls}
                      value={state.d1.databaseName}
                      onChange={(e) =>
                        patch('d1', { databaseName: e.target.value })
                      }
                    />
                  </Field>
                  <Field label="Binding 名称">
                    <input
                      className={inputCls}
                      value={state.d1.binding}
                      onChange={(e) => patch('d1', { binding: e.target.value })}
                    />
                  </Field>
                </div>
              )}
              <p className="rounded-lg bg-emerald-50 p-3 text-xs leading-relaxed text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400">
                无需手动创建：首次部署时 wrangler 会按名称自动创建 D1 数据库{' '}
                <code className="text-emerald-600 dark:text-emerald-300">
                  {state.d1.databaseName}
                </code>
                ，应用首次访问会自动建表。
              </p>
            </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="桶名">
                    <input
                      ...
                      onChange={(e) =>
                        patch('r2', { bucketName: e.target.value })
                      }
                    />
                  </Field>
                  <Field label="Binding 名称">
                    <input
                      className={inputCls}
                      value={state.r2.binding}
                      onChange={(e) => patch('r2', { binding: e.target.value })}
                    />
                  </Field>
                </div>
              )}
          ...
          {current.key === 'gdrive' && (
            <div className="space-y-4">
              <SectionTitle
                ...
                title="Google Drive 挂载"
                desc="原生 OAuth，授权后以云盘形式挂载"
              />
              <Toggle
                ...
                <div className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="Client ID">
                      <input
                        className={inputCls}
                        value={state.gdrive.clientId}
                        onChange={(e) =>
                          patch('gdrive', { clientId: e.target.value })
                        }
                    ...
                    <Field label="Client Secret">
                      <input
                        className={inputCls}
                        type="password"
                        value={state.gdrive.clientSecret}
                        onChange={(e) =>
                          patch('gdrive', { clientSecret: e.target.value })
                        }
                      />
                    </Field>
                  </div>
                  <Field
                    label="回调地址"
                    hint="必须与 Google 控制台 Authorized redirect URIs 完全一致"
                  >
                    <input
                      className={inputCls}
                      value={state.gdrive.redirectUri}
                      onChange={(e) =>
                        patch('gdrive', { redirectUri: e.target.value })
                      }
                      placeholder="https://<worker域名>/api/gdrive-oauth"
                    />
                  </Field>
                  <button
                    onClick={() => {
                      const host = state.cloudflare.workerName
                        ? `${state.cloudflare.workerName}.workers.dev`
                        : '';
                      if (host)
                        patch('gdrive', {
                          redirectUri: `https://${host}/api/gdrive-oauth`,
                        });
                    }}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="用户名">
                    <input
                      ...
                      onChange={(e) =>
                        patch('webdav', { username: e.target.value })
                      }
                    />
                  </Field>
                  <Field label="密码">
                    <input
                      className={inputCls}
                      type="password"
                      value={state.webdav.password}
                      onChange={(e) =>
                        patch('webdav', { password: e.target.value })
                      }
                    />
                  </Field>
                </div>
          ...
          {current.key === 'github' && (
            <div className="space-y-4">
              <SectionTitle
                ...
                title="Cloudflare Git 集成部署"
                desc="连接 GitHub 仓库后，push 即自动构建部署，无需任何 Secrets"
              />
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="仓库 Owner">
                  <input
                    className={inputCls}
                    value={state.github.repoOwner}
                    onChange={(e) =>
                      patch('github', { repoOwner: e.target.value })
                    }
                ...
                <Field label="仓库名">
                  <input
                    className={inputCls}
                    value={state.github.repoName}
                    onChange={(e) =>
                      patch('github', { repoName: e.target.value })
                    }
              ...
              <div className="rounded-lg bg-zinc-50 p-4 text-sm dark:bg-zinc-800">
                <ol className="list-decimal list-inside space-y-2 text-zinc-600 dark:text-zinc-300">
                  <li>
                    ...
                    → Workers 和 Pages → 创建 → Workers → 连接 Git 仓库
                  </li>
                  <li>
                    选择仓库
                    {state.github.repoOwner && state.github.repoName ? (
                      <>
                        {' '}
                    ...
                    ，监听分支 <code className="font-mono text-xs">main</code>
                  </li>
                  <li>
                    在构建设置中填写：
                    <ul className="mt-1 ml-5 list-disc list-inside text-xs text-zinc-400 dark:text-zinc-500">
                      <li>
                        构建命令:{' '}
                  ...
                  <li>
                    把下方生成的{' '}
                    <code className="font-mono text-xs">wrangler.jsonc</code>{' '}
                    提交到仓库，然后{' '}
                    <code className="font-mono text-xs">
              ...
              <div className="flex items-start gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs leading-relaxed text-emerald-700 dark:border-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-400">
                ...
                <p>
                  D1 数据库与 R2 桶在首次部署时自动创建；默认管理员{' '}
                  <code className="font-mono">admin / changeme</code>。
                  上线后请在 Cloudflare 控制台 → Settings → Variables and
                  Secrets 中修改{' '}
          ...
          {current.key === 'output' && (
            <div className="space-y-5">
              <SectionTitle
                ...
                title="生成配置"
                desc="预览 wrangler.jsonc 与完整部署命令，复制或下载"
              />
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => copy('json')}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 px-2.5 py-1 text-xs font-medium text-zinc-500 transition hover:text-zinc-700 dark:border-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200"
                    >
                      {copied === 'json' ? (
              ...
              <div className="flex items-start gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs leading-relaxed text-emerald-700 dark:border-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-400">
                ...
                <p>
                  已自动保存草稿（本地浏览器）。把生成的{' '}
                  <code className="font-mono">wrangler.jsonc</code>{' '}
                  提交到仓库并推送，Cloudflare 会自动构建部署；
                  部署后管理员在「存储管理」页挂载各存储，R2 桶会自动出现。
                </p>
              </div>
            </div>
          )}
