...
export async function loader({ request, params, context }: Route.LoaderArgs) {
  ...
  try {
    ...
    return Response.json(
      {
        error:
          error instanceof Error ? error.message : '存储配置无效，请检查后重试',
      },
      { status: 400 },
    );
  }
  ...
  if (action === 'list' || (!action && !isInlineImageRequest)) {
    try {
      const result = await withClientState(client, db, storageId, () =>
        client.listObjects(path),
      );
      return Response.json({
  ...
  if (isInlineImageRequest) {
    try {
      ...
      const response = await withClientState(client, db, storageId, () =>
        client.getObject(
          path,
  ...
  if (action === 'signed-url') {
    try {
      ...
      const signedUrl = await withClientState(client, db, storageId, () =>
        client.getSignedUrl(path),
      );
      await logAudit(db, {
  ...
  if (action === 'info') {
    try {
      const info = await withClientState(client, db, storageId, () =>
        client.headObject(path),
      );
      if (!info) {
...
export async function action({ request, params, context }: Route.ActionArgs) {
  ...
  if (method === 'POST' && action === 'multipart-init') {
    try {
      ...
      if (
        body.size !== undefined &&
        (body.size <= 0 || body.size > MAX_FILE_SIZE)
      ) {
        return Response.json(
          { error: '文件大小超出允许范围（最大 50GB）' },
          { status: 400 },
        );
      }
      if (
        body.chunkSize !== undefined &&
        (body.chunkSize < MIN_CHUNK_SIZE || body.chunkSize > MAX_CHUNK_SIZE)
      ) {
        return Response.json(
          { error: '分片大小需在 1MB-5GB 之间' },
          { status: 400 },
        );
      }
      const uploadId = await withClientState(client, db, storageId, () =>
        client.initiateMultipartUpload(path, contentType, {
          size: body.size,
  ...
  if (method === 'POST' && action === 'multipart-urls') {
    try {
      ...
      if (
        ...
        return Response.json(
          { error: 'uploadId and partNumbers are required' },
          { status: 400 },
        );
      }
      // 分片号去重并限制数量/范围，防一次请求生成海量签名 URL 耗尽 CPU
      const partNumbers = Array.from(new Set(body.partNumbers)).filter(
        (n) => Number.isInteger(n) && n >= 1 && n <= 10000,
      );
      if (partNumbers.length === 0) {
        return Response.json(
          { error: '分片号必须为 1-10000 的整数' },
          { status: 400 },
        );
      }
      if (partNumbers.length > 2000) {
        return Response.json(
          { error: '单次最多请求 2000 个分片签名' },
          { status: 400 },
        );
      }
      ...
      const urls = await withClientState(client, db, storageId, async () => {
        ...
        for (const partNumber of partNumbers) {
          try {
            result[partNumber] = await client.getSignedUploadPartUrl(
              path,
              uploadId,
              partNumber,
            );
  ...
  if (method === 'POST' && action === 'rename') {
    try {
      ...
      const canDirectRename =
        typeof (
          client as {
            renameObject?: (path: string, name: string) => Promise<void>;
          }
      ...
      if (isDirectory) {
        ...
        // Rename single file
        await withClientState(client, db, storageId, () =>
          client.copyObject(path, newPath),
        );
        await withClientState(client, db, storageId, () =>
  ...
  if (method === 'POST' && action === 'move') {
    try {
      ...
      const targetDir = destPath.endsWith('/')
        ? destPath
        : destPath
          ? destPath + '/'
          : '';
      ...
      const canDirectMove =
        typeof (
          client as {
            moveObject?: (path: string, destPath: string) => Promise<void>;
          }
        ).moveObject === 'function';
      if (canDirectMove) {
        await withClientState(client, db, storageId, () =>
  ...
  if (method === 'POST' && action === 'fetch') {
    try {
      ...
      const declaredLength = Number(
        remoteResponse.headers.get('content-length') || '0',
      );
      ...
      await withClientState(client, db, storageId, () =>
        client.putObject(
          uploadPath,
          bodyBuffer.buffer as ArrayBuffer,
          contentType,
        ),
      );
      await logAudit(db, {
  ...
  if (method === 'POST' || method === 'PUT') {
    ...
    const maxBytes = GIT_TYPES.has(storage.type)
      ? getGitMaxFileBytes(storage.type)
      : 50 * 1024 * 1024 * 1024;
    const maxLabel = GIT_TYPES.has(storage.type)
      ? getGitMaxFileLabel(storage.type)
      : '50GB';
    if (declared > 0 && declared > maxBytes) {
      return Response.json(
        { error: `文件大小超出 ${storage.type} 上限（最大 ${maxLabel}）` },
        { status: 413 },
      );
    }
    ...
    try {
      ...
      await withClientState(client, db, storageId, () =>
        client.putObject(path, bodyBuffer, contentType),
      );
      await logAudit(db, {
        action: 'file.upload',
        userType,
  ...
  if (method === 'DELETE') {
    ...
    if (action === 'rmdir') {
      try {
        ...
        try {
          await withClientState(client, db, storageId, () =>
            client.deleteObject(path.endsWith('/') ? path : path + '/'),
          );
        } catch {
