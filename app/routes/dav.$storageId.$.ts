...
function generatePropfindResponse(
  ...
  const collectionHref = (href: string) =>
...
async function validateWebdavAuth(
  ...
  try {
    ...
    const webdavPassword =
      env.WEBDAV_PASSWORD || env.ADMIN_PASSWORD || 'changeme';

    // 恒定时间比较，防时序侧信道
    return (
      timingSafeEqual(username, webdavUsername) &&
      timingSafeEqual(password, webdavPassword)
    );
  } catch {
    return false;
  }
  ...
  if (['PUT', 'DELETE', 'MKCOL', 'COPY', 'MOVE'].includes(method)) {
    ...
    if (method === 'COPY') {
      try {
        ...
        // 防穿越：目标路径同样拒绝 .. / . 段
        if (
          destPath.split('/').some((seg) => seg === '..' || seg === '.') ||
          /[\u0000-\u001f]/.test(destPath)
        ) {
        ...
        await withClientState(client, db, storageId, () =>
          client.copyObject(path, destPath),
        );
        return new Response(null, { status: 201 });
      } catch (error) {
...
${result.objects
  ...
  .join('\n')}
      ...
      const contentType =
        response.headers.get('content-type') || getContentType(path);
