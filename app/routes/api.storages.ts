...
function sanitizeStorageForClient<T extends { config?: Record<string, any> }>(
  storage: T,
): T {
  ...
  if (method === 'POST') {
    ...
    if (actionType === 'login') {
      ...
      if (!isValid) {
        ...
        const adminUser = (
          context.cloudflare.env as unknown as Record<
            string,
            string | undefined
          >
        ).ADMIN_USERNAME;
    ...
    try {
      ...
      return Response.json({
        storage: sanitizeStorageForClient({
          ...safeStorage,
          secretAccessKey: '***',
        }),
      });
