...
export async function action({ request, context }: { request: Request; context: { cloudflare: { env: Env } } }) {
  ...
  try {
    ...
    const session = { token, casCookie: jar.header(), created: Math.floor(Date.now() / 1000) };
    const ttl = QUARK_QR_TTL_SEC;
    await db
      .prepare(
        `INSERT OR REPLACE INTO ${ACTION_SESSION} (id, payload, expires_at) VALUES (?, ?, ?)`
      )
      .bind("singleton", JSON.stringify(session), Math.floor(Date.now() / 1000) + ttl)
      .run();
...
export async function loader({ request, context }: { request: Request; context: { cloudflare: { env: Env } } }) {
  ...
  const row = await db.prepare(`SELECT payload, expires_at FROM ${ACTION_SESSION} WHERE id = ?`).bind("singleton").first<{ payload: string; expires_at: number }>();
  if (!row) {
    return Response.json({ error: "扫码会话已过期，请重新获取二维码" }, { status: 400 });
  }
  const serverSession = JSON.parse(row.payload) as { token: string; casCookie: string; created: number };
  ...
  if (now > (row.expires_at || 0)) {
    try { await db.prepare(`DELETE FROM ${ACTION_SESSION} WHERE id = ?`).bind("singleton").run(); } catch { /* 清理失败不影响主流程 */ }
    return Response.json({ status: "expired", message: "二维码已过期，请重新获取" });
  }
  ...
  try {
    ...
    if (result.status === "success") {
      try {
        await db.prepare(`DELETE FROM ${ACTION_SESSION} WHERE id = ?`).bind("singleton").run();
      } catch { /* 清理失败不影响主流程 */ }
    }
