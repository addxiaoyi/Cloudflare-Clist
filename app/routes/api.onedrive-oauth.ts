...
const ONEDRIVE_OAUTH_ENDPOINTS: Record<
  string,
  { oauth: string; token: string; scope: string }
> = {
  global: {
...
async function verifyState(
  secret: string,
  state: string,
): Promise<{ storageId: number; region: string } | null> {
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  try {
    ...
    if (!oauth.clientId || !oauth.clientSecret) {
      return redirectHome(
        false,
        '未配置 ONEDRIVE_CLIENT_ID / ONEDRIVE_CLIENT_SECRET',
      );
    }
    ...
    const host =
      ONEDRIVE_OAUTH_ENDPOINTS[region] || ONEDRIVE_OAUTH_ENDPOINTS.global;
    ...
    formData.append(
      'redirect_uri',
      oauth.redirectUri || `${url.origin}/api/onedrive-oauth`,
    );
    ...
    await updateStorage(db, storageId, {
      config: {
        ...
        ...(storage.config?.root_folder_path
          ? { root_folder_path: storage.config.root_folder_path }
          : {}),
        ...(storage.config?.chunk_size
          ? { chunk_size: storage.config.chunk_size }
          : {}),
        ...(storage.config?.custom_host
          ? { custom_host: storage.config.custom_host }
          : {}),
      },
      saving: {
    ...
    return redirectHome(
      false,
      err instanceof Error ? err.message : '授权保存失败',
    );
  }
}
  ...
  if (body.action === 'start') {
    ...
    if (!oauth.clientId || !oauth.clientSecret) {
      return Response.json(
        {
          error:
            '未配置 ONEDRIVE_CLIENT_ID / ONEDRIVE_CLIENT_SECRET，无法发起授权',
        },
        { status: 400 },
      );
    }
    ...
    const redirectUri =
      oauth.redirectUri || `${new URL(request.url).origin}/api/onedrive-oauth`;
