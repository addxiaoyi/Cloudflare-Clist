...
export async function action({
  ...
  if (!storage || storage.type !== 'r2-oauth') {
    return Response.json(
      { error: 'Storage not found or not r2-oauth type' },
      { status: 404 },
    );
  }
  ...
  if (!oauth.clientId || !oauth.clientSecret) {
    return Response.json(
      { error: 'CF_CLIENT_ID / CF_CLIENT_SECRET 未配置' },
      { status: 500 },
    );
  }
...
export async function loader({
  ...
  try {
    ...
    const redirectUri =
      oauth.redirectUri || `${new URL(request.url).origin}/api/r2-oauth`;
      ...
      await updateStorage(db, storageId, {
        config: {
          ...
          cloudflare_refresh_token:
            ...
            storage.config?.cloudflare_refresh_token ||
            '',
