...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  try {
    ...
    formData.append(
      'redirect_uri',
      oauth.redirectUri || `${url.origin}/api/gdrive-oauth`,
    );
    ...
    await updateStorage(db, storageId, {
      config: {
        ...
        // 已有手动配置保留
        ...(storage.config?.root_folder_id
          ? { root_folder_id: storage.config.root_folder_id }
          : {}),
        ...(storage.config?.order_by
          ? { order_by: storage.config.order_by }
          : {}),
        ...(storage.config?.order_direction
          ? { order_direction: storage.config.order_direction }
          : {}),
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (body.action === 'start') {
    ...
    if (!storage || storage.type !== 'gdrive') {
      return Response.json(
        { error: '请先保存一个 Google Drive 类型的存储' },
        { status: 400 },
      );
    }
    if (!oauth.clientId || !oauth.clientSecret) {
