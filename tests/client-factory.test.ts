...
describe("createClient Git Dispatch", () => {
  const TYPE_CONFIGS: Array<{ type: string; token: string; maxBytes: number }> = [
    { type: "github", token: "ghp_test", maxBytes: 100 * 1024 * 1024 },
    { type: "gitlab", token: "glpat_test", maxBytes: 10 * 1024 * 1024 },
    { type: "gitea", token: "gitea_test", maxBytes: 100 * 1024 * 1024 },
    { type: "gitee", token: "gitee_test", maxBytes: 10 * 1024 * 1024 },
  ];

  test.each(TYPE_CONFIGS)("createClient(type='${this.type}') 返回 GitRepositoryClient 实例", ({ type, token, maxBytes }) => {
