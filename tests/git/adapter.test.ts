...
import { GIT_TYPES, getAdapter, getGitMaxFileBytes, getGitMaxFileLabel, isGitStorageType } from "~/lib/git/registry";
...
describe("GitHub Adapter", () => {
  ...
  test("buildConnection 缺少 token 抛出错误", () => {
    expect(() => adapter.buildConnection({ repo: "owner/repo", token: "" })).toThrow("GitHub 存储需填写 Personal Access Token");
  });
...
describe("GitLab Adapter", () => {
  ...
  test("buildConnection 使用默认分支 main", () => {
    const conn = adapter.buildConnection({ repo: "group/project", token: "glpat_xxx" });
    expect(conn.branch).toBe("main");
  });
...
describe("Gitee Adapter", () => {
  ...
  test("buildConnection 缺少 token 抛出错误", () => {
    expect(() => adapter.buildConnection({ repo: "owner/repo", token: "" })).toThrow("Gitee 存储需填写私人令牌");
  });
});
